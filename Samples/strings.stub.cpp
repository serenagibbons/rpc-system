...
string __concat(string s1, string s2) {
  char doneBuffer[5] = "DONE";  // to write magic value DONE + null
  string result;
  //
  // Time to actually call the function 
  //
  c150debug->printf(C150RPCDEBUG,"strings.stub.cpp: invoking concat()");
  result = concat(s1, s2);

  //
  // Send the response to the client
  //
  // If concat returned something other than void, this is
  // where we'd send the return value back.
  //
  c150debug->printf(C150RPCDEBUG,"strings.stub.cpp: returned from  concat() -- responding to client");
...
void __badFunction(char *functionName) {
  ...
  c150debug->printf(C150RPCDEBUG,"strings.stub.cpp: received call for nonexistent function %s()",functionName);
  RPCSTUBSOCKET->write(doneBuffer, strlen(doneBuffer)+1);
}
...
void dispatchFunction() {
  ...
  if (!RPCSTUBSOCKET-> eof()) {
    if (strcmp(functionNameBuffer,"concat") == 0)
      __concat("Hello ", "World");
    else
...
void getFunctionNameFromStream(char *buffer, unsigned int bufSize) {
  ...
  //
  if (readlen == 0) {
    c150debug->printf(C150RPCDEBUG,"strings.stub: read zero length message, checking EOF");
    if (RPCSTUBSOCKET-> eof()) {
      c150debug->printf(C150RPCDEBUG,"strings.stub: EOF signaled on input");

    } else {
      throw C150Exception("strings.stub: unexpected zero length read without eof");
    }
  }
  ...
  //
    ...
    throw C150Exception("strings.stub: method name not null terminated or too long");
